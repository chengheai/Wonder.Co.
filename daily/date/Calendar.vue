  ...
  methods: {
    ...
    pickDate(date) {
      ...
      if (!this.SameWeek(this.newDate) ){
        ...
        // d=d.split(" ")
        const todayDate=this.todayDate.split("-")
        let weeks=0
        weeks=parseInt((d.getDate()-parseInt(todayDate[2]))/7)+1
        ...
        console.log(this.days)
        // 获得今天是几号用于跟新日期比较
        const newDates=this.newDate.split("-")
        const todayDate=this.todayDate.split("-")
        // 将新index赋给全局变量
        this.tabIndex=parseInt(newDates[2])-parseInt(todayDate[2])+index


      }


      // this.pick(this.newDate, this.index)
